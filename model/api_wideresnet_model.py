...
class decoder(nn.Module):
    def __init__(self, channel1 = 1024, channel2= 512, channel3= 64, **kwargs):
        ...
        self.tconv1 = dec_tra_conv(dw_channels= 2048, out_channels = channel1, kernel_size=4, padding=1, stride=2)
...
class my_model(nn.Module):
    def __init__(self):
        ...
        layers = models.wide_resnet50_2(pretrained=False)
        layers = list(layers.children())
        self.conv = nn.Sequential(*layers[:-2])
        self.avg = nn.AdaptiveAvgPool2d(1)
        self.conv1 = conv_bn_lrelu(in_channels=4096, out_channels= 1792, kernel_size=3,
                                   stride=1, padding=1)
        self.conv2 = conv_bn_lrelu(in_channels=1792, out_channels= 1024, kernel_size=3,
                                   stride=1, padding=1)
        self.conv3 = conv_bn_lrelu(in_channels=1024, out_channels= 2048, kernel_size=3,
                                   stride=1, padding=1)
        self.drop = nn.Dropout(p = 0.3)
        self.sigmoid = nn.Sigmoid()
        self.decoder = decoder(512, 256, 64)
...
def get_model(pretrained=False, root="./weights", **kwargs):
    if pretrained:
        model = my_model()
        model.load_state_dict(torch.load(root+"/wideresnet_api_citys.pth"))
    else:
        model = my_model()
    return model
